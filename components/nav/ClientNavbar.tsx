...
import { Button } from "@/components/ui/button";
import { RoleSwitcher } from "./RoleSwitcher";
import { useWhiteLabel } from "@/hooks/use-white-label"; // 1. Import the hook

export const ClientNavbar = ({ userName, userAvatar }: { userName: string; userAvatar?: string }) => {
  const { branding } = useWhiteLabel(); // 2. Consume the context

  return (
    <header className="flex items-center justify-between py-3 px-6 border-b bg-white shadow-sm sticky top-0 z-30">
      <div className="flex items-center gap-2">
         {/* 3. Make the logo and name dynamic */}
         {branding.logo && <img src={branding.logo} alt="Logo" className="h-8 w-8"/>}
        <h1 className="font-bold text-xl text-gray-800">{branding.platformName}</h1>
      </div>
      <div className="flex items-center gap-4">
         <RoleSwitcher />
        <span className="text-sm font-medium text-gray-700">Welcome, {userName}</span>
